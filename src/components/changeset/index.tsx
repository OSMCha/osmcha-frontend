 ...
 */
export class _Changeset extends React.PureComponent<propsType, changesetStateType> {
  ...
  zoomToAndSelect = (type: string, id: number) => {
    ...
    let bounds2d = bounds.slice(0, 4) as [number, number, number, number];
    let camera = map.cameraForBounds(bounds2d, { padding: 50, maxZoom: 18 });
    if (camera) {
      map.jumpTo(camera);
    }
