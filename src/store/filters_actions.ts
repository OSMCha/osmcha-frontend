...
export const applyFilters = (filters: filtersType, pathname?: string | null) =>
  action(FILTERS.apply, { filters, pathname });

export function* watchFilters(): any {
  yield all([
    watchLocationChange(),
    takeLatest(FILTERS.apply, applyFilterSaga as any),
  ]);
}
...
export function* filtersSaga(location: any): any {
  try {
